...
def numeric_sort_key(folder):
    m = re.search(r"(\d+)$", folder.name)
    return int(m.group(1)) if m else 0

def compare_pair(img1_path, img2_path, out_dir, set_name):
    ...
    for res in results:
        print(f"  {res['name']}: total time = {res['t_total']:.4f}s, inliers = {res['inliers']}, good matches = {res['good']}")

    faster = min(results, key=lambda r: r["t_total"])
    more_accurate = max(results, key=lambda r: r["inliers"])
    print(f"  ➤ Faster method: {faster['name']} ({faster['t_total']:.4f}s)")
    print(f"  ➤ More accurate method: {more_accurate['name']} ({more_accurate['inliers']} inliers)\n")
